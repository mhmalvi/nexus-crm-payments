...
class InvoiceController extends Controller
{
    ...
    public function generatePDF(Request $request)
    {
        try {
            ...
            $fileName = 'invoice-' . $response->customer_name . '.pdf';
            $data = [
            'price' => $response->lines->data[0]->amount,
            'company_name' => $response->customer_name,
            'inv_no'=>$request->inv_id
            ];
            ...
            // $student_invoice_exist->file_path =
            'https://crmbtob.quadque.digital/storage/app/public/invoice/'.$fileName;
